...
if ($Target -eq "autopsy") {
    ...
    $sources = @(
        (Join-Path $codeRoot "autopsy\faulty_sift_down.c")
    )
    $outputName = "binary_heap_autopsy"
} else {
    $sources = @(
        (Join-Path $codeRoot "starter\alert_priority_queue.c")
    )
...
if ($compilerKind -eq "msvc") {
    ...
    if ($testingBuild) {
        $arguments += "/DALERT_PRIORITY_QUEUE_TESTING"
    }
    if ($Sanitize) {
