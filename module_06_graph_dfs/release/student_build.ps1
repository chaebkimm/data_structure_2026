[CmdletBinding()]
param(
    [ValidateSet("starter", "self-loop", "autopsy")]
...
if ($Target -eq "autopsy") {
    ...
    if ($StudentTests) {
        throw "Self-loop test cannot be combined with -StudentTests."
    }

    $sources = @(
        (Join-Path $codeRoot "support\graph_matrix.c"),
        (Join-Path $codeRoot "support\vertex_stack.c"),
        (Join-Path $codeRoot "starter\graph_dfs.c"),
        (Join-Path $codeRoot "tests\test_self_loop.c")
    )
    $outputName = "graph_self_loop_test"
} else {
    $sources = @(
        (Join-Path $codeRoot "support\graph_matrix.c"),
        (Join-Path $codeRoot "support\vertex_stack.c"),
