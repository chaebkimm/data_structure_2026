[CmdletBinding()]
param(
    [ValidateSet("starter", "solution", "self-loop", "autopsy")]
...
if ($Target -eq "autopsy") {
    ...
    $sources = @(
        (Join-Path $codeRoot "autopsy\faulty_cycle_dfs.c")
    )
    $outputName = "graph_dfs_autopsy"
} elseif ($Target -eq "self-loop") {
    if ($Extensions -or $StudentTests) {
        throw "Self-loop cannot be combined with a normal test switch."
    }

    $sources = @(
        (Join-Path $codeRoot "support\graph_matrix.c"),
        (Join-Path $codeRoot "support\vertex_stack.c"),
        (Join-Path $codeRoot "solution\graph_dfs.c"),
        (Join-Path $codeRoot "tests\test_self_loop.c")
    )
    $outputName = "solution_self_loop"
} else {
    ...
    $sources = @(
        (Join-Path $codeRoot "support\graph_matrix.c"),
        (Join-Path $codeRoot "support\vertex_stack.c"),
        (Join-Path $codeRoot "$Target\graph_dfs.c")
    )
