...
static ProbeResult probe_for_key(const HashTable *table, uint32_t key)
{
    ...
    /*
     * TODO 1: Implement the full capacity-bounded linear-probe helper. Stop
     * at a matching key or EMPTY slot, remember the first DELETED slot for
     * insertion, wrap at capacity, and report every examined slot.
     */
    (void)table;
    (void)key;
    return result;
...
/*
 * TODO 2: Implement the insertion/update and removal mutation helpers.
 * Updates must not duplicate keys. Inserts must reuse a remembered tombstone
 * or request maintenance at the three-quarter effective-occupancy boundary.
 * Removal must write a canonical DELETED slot. Commit result structs last so
 * every error preserves caller output.
 */
...
/*
 * TODO 3: Implement transactional compaction/growth. Allocate and populate a
 * replacement, including the requested entry, before swapping table fields.
 * On any failure release only the replacement and preserve the old table.
 */
static HashTableStatus rebuild_with_entry(
    HashTable *table,
    size_t new_capacity,
