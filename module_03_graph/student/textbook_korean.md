...
소셜 네트워킹 서비스(SNS)에 새로 가입한 누리가 처음으로 유명인 미나를 팔로우한다고 생각해 보자. 준, 소라, 대는 이미 미나를 팔로우하고 있다. 이 작은 예제에서 미나는 유나 한 명을 팔로우하고, 유나는 아무도 팔로우하지 않는다. 계정마다 점을 하나씩 그리고 팔로우 관계마다 화살표를 하나씩 그려서 나타낸다.

화살표는 팔로우하는 계정에서 팔로우 대상 계정으로 향한다. 누리의 첫 팔로우를 추가한 `6 -> 0`은 누리가 미나를 팔로우한다는 뜻이다. 다음 그림은 이 팔로우를 추가한 뒤의 네트워크다.

```mermaid
flowchart LR
    nuri["6 누리 · 새 가입자"] --> mina["0 미나 · 유명인"]
    mina --> yuna["7 유나"]
    joon["1 준"] --> mina
    joon --> sora["2 소라"]
    sora --> mina
...
그림에는 화살표 열 개가 있다. `0 -> 7`, `1 -> 0`, `1 -> 2`, `2 -> 0`, `2 -> 3`, `3 -> 0`, `3 -> 1`, `4 -> 5`, `5 -> 4`, `6 -> 0`이다. 누리가 첫 팔로우를 하기 전에도 같은 여덟 계정이 있었고 화살표는 아홉 개였다.

### 팔로우 수와 팔로워 수는 어떻게 셀까?

누리에서 나가는 화살표는 `6 -> 0` 하나다. 따라서 누리의 팔로우 수는 1이다. 누리로 들어오는 화살표는 없으므로 팔로워 수는 0이다. 처음으로 누군가를 팔로우했다고 그 사람이 누리를 팔로우하는 것은 아니다.

미나로 들어오는 화살표는 준, 소라, 대, 누리에게서 온다. 미나의 팔로워 수는 4다. 미나에서 나가는 화살표는 유나를 향하는 `0 -> 7` 하나이므로 팔로우 수는 1이다.

유나에게는 미나에게서 들어오는 화살표 하나가 있고 나가는 화살표는 없다. 유나의 팔로워 수는 1이고 팔로우 수는 0이다. 아무도 팔로우하지 않는다는 것과 아무도 자신을 팔로우하지 않는다는 것은 다르다.

누리의 첫 팔로우 전후를 비교하면 누리의 팔로우 수는 0에서 1로, 미나의 팔로워 수는 3에서 4로 바뀐다. 누리의 팔로워 수는 0, 미나의 팔로우 수는 1로 유지된다.

### 누리는 화살표를 따라 어디까지 갈 수 있을까?

누리는 미나를 팔로우하고, 미나는 유나를 팔로우한다. `6 -> 0 -> 7`을 따라가면 누리에서 미나를 거쳐 유나에 도달한다. 누리가 유나를 직접 팔로우하지 않아도 가능하다.

유나에게는 나가는 화살표가 없어 여기서 멈춘다. 미나나 유나에서 화살표를 따라 누리에게 돌아갈 수도 없다. 누리가 어떤 계정에 도달할 수 있어도 그 계정에서 누리에게 돌아올 수 있는 것은 아니다.

### 어떤 계정들이 연결되어 있을까?

이번에는 다른 계정을 거쳐 서로 연락할 수 있는지 알아보자. 기록할 관계를 바꾼다. 연락 관계는 양쪽 방향으로 이용할 수 있다. 같은 여덟 계정에 다음 여덟 관계를 둔다.

```text
0—1, 0—2, 1—2, 2—3, 3—6, 3—7, 6—7, 4—5
```

앞의 팔로우와는 별도로 정한 관계다. 기존 팔로우에 반대 방향의 팔로우가 생겼다는 뜻이 아니다.

계정 하나를 **정점(vertex)**, 직접적인 관계 하나를 **간선(edge)**이라고 한다. 정점과 간선으로 관계를 나타내는 모델이 **그래프(graph)**다. 앞의 화살표는 **방향 그래프(directed graph)**를 이룬다. 연락 관계의 간선에는 방향이 없으므로 **무방향 그래프(undirected graph)**를 이룬다. 이제부터는 이 연락 그래프를 사용한다.

정점을 반복하지 않고 간선을 따라 이어지는 정점의 나열을 **경로(path)**라고 한다. 미나는 `0—2—3—7`을 따라 유나에게 도달한다. 그룹은 `{0, 1, 2, 3, 6, 7}`과 `{4, 5}`로 나뉜다. 각 그룹의 모든 정점 쌍은 경로로 연결되며, 이 성질을 유지하면서 바깥 정점을 추가할 수 없다. 이런 그룹을 **연결 요소(connected component)**라고 한다. 더 확장할 수 없다는 뜻의 **극대(maximal)** 조건이지, 크기가 가장 커야 한다는 뜻은 아니다. 간선이 없는 **고립 정점(isolated vertex)**도 혼자 연결 요소를 이룬다.

### 무방향 그래프는 어떻게 저장할까?

소라에서 이동하려면 소라와 직접 연락하는 계정들을 알아야 한다. 정점마다 직접 연결된 이웃을 모아 두는 저장 방법을 **인접 리스트(adjacency list)**라고 한다.
...
간선 하나를 양 끝 정점의 목록에 기록한다. `2—3`은 소라의 목록에 `3`, 대의 목록에 `2`를 넣는다. 두 항목은 같은 간선 번호를 가진다. 간선 여덟 개에는 항목 열여섯 개가 필요하다.

C 예제는 배열을 사용한다. `head`는 목록의 첫 항목 번호를 기록한다. 각 항목의 `to`는 이웃 번호, `next`는 다음 항목 번호를 기록한다. `-1`은 목록의 끝이다. 항목들이 배열에서 붙어 있지 않아도 번호로 이어진다. 각 항목을 번호순으로 맞는 위치에 넣어 이웃 번호의 오름차순을 유지한다.

### 연결 요소를 모두 찾으려면 어떻게 할까?

미나에서 시작하여 도달할 수 있는 모든 계정에 같은 번호를 붙이자. 이미 방문했다는 표시가 없으면 `0—1—2—0`을 계속 돌 수 있다.

1. 모든 계정을 미방문 상태로 둔다. 번호가 가장 작은 미방문 계정을 골라 새 연결 요소 번호를 붙인다.
2. 이웃을 번호순으로 확인한다. 미방문 이웃을 만나면 즉시 표시하고 같은 연결 요소 번호를 붙인다. 그 이웃을 대상으로 함수를 재귀 호출한다.
3. 호출한 함수가 이웃 쪽의 탐색을 모두 마치고 돌아온 뒤, 원래 계정의 나머지 이웃을 확인한다. 이미 표시한 이웃은 건너뛴다.
4. 시작 계정의 호출이 끝나면 다음 미방문 계정에서 새 연결 요소를 시작한다.

첫 연결 요소에서는 `0, 1, 2, 3, 6, 7` 순서로 계정을 처음 발견한다. 다음은 `4`에서 시작하여 `5`를 발견한다. 미방문 계정이 없으므로 연결 요소는 두 개다. 고립 정점에서 시작한 호출은 바로 끝나지만 그 정점에도 연결 요소 번호를 붙인다.

### 어떤 계정이 사라지면 연결된 그룹이 나뉠까?

같은 연결 요소에 속해도 계정 하나가 사라지면 연결이 끊길 수 있다. 소라와 소라에 닿은 간선을 모두 지워 보자. 미나와 준은 대, 누리, 유나에게 도달할 수 없다. 대를 지워도 원래 연결 요소가 나뉜다. 미나를 지우면 준과 소라는 계속 연결된다.

삭제했을 때 그래프의 연결 요소 수가 증가하는 정점을 **단절점(cut vertex, cut node, articulation point)**이라고 한다. 예제의 단절점은 소라 `2`와 대 `3`이다. 간선 `2—3`만 지워도 연결 요소 수가 증가한다. 이런 간선을 **브리지(bridge)**라고 한다. `4—5`도 브리지다.

정점을 하나씩 지워 보며 매번 연결 요소를 구하면 같은 작업을 반복한다. 탐색한 가지에 다른 복귀 경로가 있는지 기록해 두자.

### dfn과 low에는 무엇을 기록할까?

앞의 탐색 순서를 다시 사용한다. 재귀 호출로 새 계정을 처음 발견하면 그 계정은 호출한 계정의 자식이 된다. 이런 최초 발견 연결만 모으면 연결 요소마다 트리가 생긴다. 나머지 간선은 자손과 먼저 발견한 조상을 연결할 수 있다.
...
`2`에서는 간선 `2—0`으로 발견 번호 1에 도달한다. `7`에서는 간선 `7—3`으로 발견 번호 4에 도달한다. 호출이 돌아오면 이 값을 부모 쪽으로 전달한다.
...
소라의 자식 대는 `4 >= 3`을 만족하므로 소라는 단절점이다. 대의 자식 누리는 `4 >= 4`를 만족하므로 대도 단절점이다. 같은 경우도 포함한다. 대로 돌아가는 길이 있어도 대 자체를 지우면 그 길은 쓸 수 없다.

시작 정점인 루트에는 부모 쪽이 없다. 루트가 새 자식을 둘 이상 발견했을 때만 단절점이다. 이웃 수가 아니라 처음 발견한 자식 수를 센다. 미나의 이웃은 둘이지만 새 자식은 준 하나다. 준의 호출에서 소라까지 발견하기 때문이다.

자식으로 향하는 간선이 브리지인 조건은 `low[v] > dfn[u]`다. 두 값이 같으면 그 간선 없이도 `u`로 돌아갈 수 있다. 따라서 `2—3`은 브리지지만 `3—6`은 브리지가 아니다.

### 이중 연결 블록은 어떻게 나눌까?

두 삼각형은 각각 계정 하나를 지워도 남은 계정끼리 연결된다. 이런 성질을 유지하는 극대 부분을 구하자.
...
`C2`는 소라, `C3`는 대다. 단절점이 아닌 정점들은 해당 블록 노드 안에 남는다. B4는 노드 하나인 트리를 이룬다. 원래 그래프가 연결되어 있지 않으면 여러 트리의 모음인 **포리스트(forest)**를 얻는다. 고립 정점 블록도 혼자 트리 하나가 된다.
...
예제는 정점 여덟 개와 간선 여덟 개를 사용한다. 연락 여부만 저장하며 간선에 수치 가중치를 두지 않는다.

## 효율 계산해 보기

### 연결 요소를 모두 찾는 데 얼마나 많은 작업이 필요할까?

예제에서는 계정 여덟 개를 표시하고 이웃 항목 열여섯 개를 확인한다. 일반적으로 사용 중 정점 `V`개와 인접 항목 `2E`개를 처리한다. 방문 표시의 초기화와 다음 시작점을 찾는 반복문도 정점 `V`개를 확인한다. 전체 시간은 `O(V + E)`다.

### 타잔 알고리즘은 얼마나 많은 작업이 필요할까?

같은 여덟 계정과 열여섯 항목에서 발견 번호와 복귀 정보를 구한다. 각 간선은 대기 배열에 한 번 들어갔다가 한 번 나온다. 블록별로 정점 표시를 사용하면 같은 끝점을 반복 출력하지 않고 블록의 소속 정점과 블록-단절점 포리스트를 만들 수 있다. 전체 작업량은 출력까지 포함하여 `O(V + E)`다. 정점을 하나씩 삭제해 볼 필요가 없다.
...
그래프는 정점 용량 `M = 16`과 간선 용량 `L = 120`을 확보한다. 인접 항목 용량은 `2L = 240`이다. 분석과 결과 배열도 `M + L`에 비례하는 고정 공간을 확보한다. 따라서 확보 공간은 점유 상태와 관계없이 `O(M + L)`이다. 예제에서 실제 사용하는 정점은 여덟 개이고 인접 항목은 열여섯 개다.
...
2. 그래프를 초기화한다. 주어진 순서대로 연락 간선 여덟 개를 검사하고 추가한다.
...
시작 정점이 아닌 정점과 시작 정점의 단절점 판정은 나뉩니다. 자식이 하나뿐인 시작 정점으로 돌아와도 블록을 꺼냅니다. 따라서 미나는 단절점이 아니어도 B3을 완성합니다.
...
입력에는 본문에서 사용한 무방향 간선 여덟 개가 들어 있습니다. 간선을 추가한 다음 그래프를 분석하고 기록을 출력합니다. 고정 배열만 사용하므로 동적 메모리 할당이나 해제가 필요하지 않습니다.
