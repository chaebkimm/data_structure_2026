# Stage C — Investigation Worksheet: Arrival Order in Fixed Storage
...
blank. List all four physical positions and label each `LIVE`, `STALE`, or
`UNUSED`. Write logical items from front to back.
...
|---|---|---|
| initialize capacity 4 in a Queue variable whose four fields are zero | | |
| initialize capacity 65 in a Queue variable whose four fields are zero | | |
| initialize an already initialized positive-capacity Queue | | |
| `enqueue` into a full valid Queue | | |
| `peek` at an empty valid Queue; output began as `-7` | | |
| `dequeue` from an empty valid Queue; output began as `-7` | | |
| `dequeue` with a `NULL` output pointer | | |
| validate a Queue with `size > capacity` | | |
...
|---|---:|---:|---|
| shifting array Queue | | | |
| fixed circular Queue | | | |
| linked Queue with front and back pointers | | | |
