...
| --- | --- | --- |
| 01 | C variables, functions, arrays, loops, conditionals | Data structures, contiguous memory, lists, fixed-capacity ArrayLists, indexes, elements, size versus capacity, invariants, indexed access, linear search, and unchanged state after rejected operations |
| 02 | Fixed arrays, array-based lists, bounds checks, invariants | Expression trees as an application of generic binary trees, operator/operand side order, character constants as simplified integer labels, addresses, pointers, `struct`, `&`, `.` and `->`, `NULL`, hierarchy, binary tree, node, root, parent as a relationship, left/right child positions, sibling, ancestor, descendant, leaf, path, depth, height, subtree, local-node lifetime, no-sharing and no-cycle rules, recursion and its stopping case, current-node/left/right search, and subtree field clearance |
| 03 | Fixed arrays, size versus capacity, indexes, structs, trees, paths, cycles, and valid conditions | Graphs, vertices, edges, direction and weight, in-degree and out-degree, self-loops, edge lists, adjacency lists, adjacency matrices, isolated vertices, connected components of undirected graphs, symmetric matrices, two-dimensional arrays, `size_t`, and a fixed directed unweighted integer-matrix implementation |
| 04 | Fixed-capacity array-based lists, node pointers and links, valid conditions, keeping state after rejection, amount of work, simple recursive calls | Growable array storage, allocation and release, ownership, checked expansion, doubling, and average work over many additions; Stack behavioral rules, taking out the last added value first, `push`, `pop`, `peek`, bracket checking. (The lab introduces `realloc`; running function call storage space and depth-first search are mentioned by name only as a preview) |
| 05 | Binary trees, simple recursive current-node/left/right search and field clearance, stacks, allocation from Chapter 4 | Binary-search ordering, depth-first search, named preorder/inorder/postorder traversals, state remembered by running calls, call space `O(h)`, and construction and postorder destruction of separately allocated nodes |
