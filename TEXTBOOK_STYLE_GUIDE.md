...
| 04 | Fixed arrays, character and integer values, indexes versus counts, active prefixes, bounds, loops, conditionals, simple function calls, and expression precedence | Stack and LIFO behavior, `push`, `peek`, `pop`, underflow, and the runtime call-stack comparison; global `int stack[10]` with forward growth, item count and next insertion index `size`, active prefix, post-increment push and pre-decrement pop; unchecked operations with separate full/empty predicates; direct two-Stack calculation as motivation before sequential infix-to-postfix conversion and postfix evaluation (`1-2*3+4 -> 123*-4+ -> -1`) using the same global Stack; `eq`, `eq_re`, exactly seven tokens, local output position `pos`, a bottom null sentinel drained into the string terminator; phase-entry `size` resets, right-before-left operand pops, and explicit valid-input assumptions |
