...
| 04 | Fixed arrays, character and integer values, indexes versus counts, active prefixes, bounds, loops, conditionals, simple function calls, and expression precedence | Stack and LIFO behavior, `push`, `peek`, `pop`, underflow, and the runtime call-stack comparison; global `char stack[10]` with forward growth, next insertion index and count `pos`, active prefix, full-push no-op and empty-read null sentinel; separate infix-to-postfix conversion and integer postfix evaluation (`1-2*3+4 -> 123*-4+ -> -1`); separate global character `pos`, postfix `size`, and local numeric `pos`; right-before-left operand pops and explicit valid-input assumptions |
