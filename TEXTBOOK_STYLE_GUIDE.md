...
| 03 | Fixed arrays, size versus capacity, indexes, structs, trees, paths, cycles, and valid conditions | Graphs, vertices, edges, direction and weight, in-degree and out-degree, self-loops, edge lists, adjacency lists, adjacency matrices, isolated vertices, maximal connected components of undirected graphs, weakly and strongly connected components of directed graphs, conceptual feed candidate selection versus ranking, symmetric matrices, two-dimensional arrays, `size_t`, and a fixed directed unweighted integer-matrix implementation |
