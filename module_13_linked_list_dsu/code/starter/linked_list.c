...
/*
 * TODO 1: Complete the linked-list ownership cluster.
 *
 * Part A — push_front: reject invalid/full state, allocate before changing
 * the list, then link and commit.
 * Part B — remove_first: preserve the output on errors, remove only the
 * first match, and save every needed link before releasing the victim.
 * Part C — destroy: save next before each release and restore exact zero.
 */
LinkedListStatus linked_list_push_front(LinkedList *list, int value)
{
    (void)list;
    (void)value;
    (void)linked_node_allocate;
    return LINKED_LIST_INVALID_STATE;
}
