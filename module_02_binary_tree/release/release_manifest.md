...
This stage contains no node struct, invariant answer, code, solution, or
instructor material.
...
- vocabulary reference containing the formal tree rules.

Withhold the instructor explanation until each student has completed the
initial response, including approved extended-time or asynchronous
equivalents.
...
This stage prompts tree, BST, and Tree-to-Graph reasoning without supplying
the answer key.
...
- lab, rubric, evidence template, and Tree Structure Autopsy;
...
- solution-independent faulty-link autopsy.
...
Before release, build the reference core tests and the isolated autopsy with
the commands documented in `../code/README.md`. Run warning-enabled and
