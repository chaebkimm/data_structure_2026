...
| parent | A node directly above another node; no upward field is stored |
...
| `char data` | The field storing one digit or operator character |
| child index | An integer selecting a node in the nodes array |
| `nodes[root].data` | Select the node at root, then read its data field |
...
| nonmutation | Leaving stored fields and shared state unchanged during an operation |
