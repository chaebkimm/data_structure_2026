...
Vertices and outgoing neighbors:
...
For each edge, state the destination state when checked, required action,
and fields changed:
...
Use vertices 0 through 6 and these exact edges:
...
3. {3,4}
4. {4,5}
5. Vertex 6 is isolated.
```

Scan possible roots from 0 through 6. Keep one shared discovered array.
...
```text
matrix representation storage:
adjacency-list representation storage:
matrix public BFS time:
adjacency-list public BFS time:
matrix BFS auxiliary space:
adjacency-list BFS auxiliary space:
```
