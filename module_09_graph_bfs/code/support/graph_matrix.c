...
{
    GraphStatus status = graph_check_container(graph);
...
{
    GraphStatus status = graph_check_vertex(graph, vertex);
    size_t other;
...
{
    GraphStatus status = graph_check_pair(
        graph,
        from_vertex,
        to_vertex
    );
...
)
{
    ...
    for (other = 0U; other < graph->vertex_count; ++other) {
        if (graph->adjacency[vertex][other]) {
            candidate.vertices[candidate.count] = other;
            candidate.count += 1U;
        }
    }
